    ...
    async componentWillMount() {
        ...
        src.map((item, i) => {
            ...
            allSubCourseComponent.push(new Promise(async (resolve, reject) => {
                ...
                if (i == this.props.now) {
                    resolve(
                        <Button className='sidebarHover' style={{display:'block',zIndex:100,position:'absolute',width:'100%',minWidth:'100%'}} href={ipList.frontend+'/learning/'+this.props.courseid+'/'+item.subcourseid} >
                        <tr>
